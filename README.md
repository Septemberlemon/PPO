...
这样对任意 $0 \leq k < \infty$ ， $\delta_k$ 都有定义

下面定义 $\hat{A}^n_t$ ，它意为从第**t**步开始对后续**n**项 $\delta$ 的加权和：
...
&=\gamma^n V(s_{t+n})-V(s_t)+\sum_{k=t}^{t+n-1} \gamma^{k-t} r(k)\qquad \qquad (1)
\end{align}
$$
...
=&\nabla_\theta J(\theta)\qquad \qquad (2)
\end{align}
$$
